...
public class StringStuff {
   ...
   */
   public static boolean containsVowel( String s ) {
      char c;                                                                             //initialize c to be used to store each char of the string
   ...
   */
   public static String evensOnly( String s ) {
      ...
      for ( int i = 0; i < s.length(); i++ ) {
         int letternum = Character.getNumericValue(s.charAt(i));
         if ( letternum % 2 == 1 && letternum > 9 && letternum < 36 ) {
            result += s.charAt(i);
         }
      }
      return result;
   ...
   */
   public static String oddsOnly( String s ) {
      ...
      for ( int i = 0; i < s.length(); i++ ) {
         int letternum = Character.getNumericValue(s.charAt(i));
         if ( letternum % 2 == 0 && letternum > 9 && letternum < 36 ) {
            result += s.charAt(i);
         }
      }
      return result;
   ...
   public static String evensOnlyNoDupes( String s ) {
      ...
      for ( int i = 0; i < evens.length(); i++ ) {
         for (  int j = 0; j < result.length(); j++ ) {
            if ( evens.charAt(i) == result.charAt(j) ) {
               dupe = true;
            }
         }
         if ( !dupe ) {
            result += evens.charAt(i);
         }
         dupe = false;
      }
      return result;
   ...
   */
   public static String oddsOnlyNoDupes( String s ) {
      ...
      for ( int i = 0; i < odds.length(); i++ ) {
         for (  int j = 0; j < result.length(); j++ ) {
            if ( odds.charAt(i) == result.charAt(j) ) {
               dupe = true;
            }
         }
         if ( !dupe ) {
            result += odds.charAt(i);
         }
         dupe = false;
      }
      return result;
